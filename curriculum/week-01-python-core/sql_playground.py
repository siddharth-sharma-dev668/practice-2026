...
print("     the SQL STANDARD, just not for SQLite specifically. The rule you learned")
print("     is real: WHERE executes BEFORE SELECT (FROM -> WHERE -> GROUP BY -> HAVING")
print("     -> SELECT -> ORDER BY is the true execution order, not the order you type")
print("     clauses in), so 'raised' shouldn't exist yet when WHERE runs. PostgreSQL,")
print("     MySQL and SQL Server all reject this exact query with a missing-column")
print("     error. SQLite is unusually permissive and resolves it anyway. The lesson")
print("     is bigger than the syntax: your LOCAL dev database (SQLite, zero setup)")
print("     can silently accept something your DEPLOYED database (Postgres, from Week 3)")
print("     will reject. Re-run this exact query once Pulse has real Postgres and see")
print("     for yourself - don't take this file's word for it either.\n")
